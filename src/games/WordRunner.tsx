...
export function WordRunner({ profileId, layoutId, onExit }: GameProps) {
  ...
  const start = () => { setScore(0); scoreRef.current = 0; setRemaining(DURATION); setTyped(""); nextWord(); hitsRef.current = 0; triesRef.current = 0; capsRef.current = []; lastKeyRef.current = performance.now(); setOver(false); setRunning(true); };
  ...
  const onChange = (value: string) => {
    ...
    if (value === word) { const pts = word.length; setScore((s) => { scoreRef.current = s + pts; return s + pts; }); setTyped(""); nextWord(); return; }
    ...
    <div className="space-y-3">
      ...
      <div className="grid min-h-[220px] place-items-center rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
        ...
        {running && (<div className="w-full max-w-md text-center">
          ...
          <input autoFocus value={typed} onChange={(e) => onChange(e.target.value)} onPaste={(e) => e.preventDefault()} className={`mt-4 w-full rounded-md border border-slate-300 p-3 text-center text-xl ${hindi ? "font-krutidev" : "font-mono"} dark:border-slate-700 dark:bg-slate-800`} placeholder="Type the word" />
        </div>)}
      </div>
