...
export function MappingValidatorPage() {
  ...
  const [summary, setSummary] = useState<KeyboardMappingVerification>({ layoutId: "kruti-dev-010", totalMappableKeys: 0, verifiedCount: 0, unverifiedCount: 0, unsupportedCount: 0, updatedAt: "" });
  ...
  const load = async (id: KeyboardLayoutId) => {
    ...
    for (const k of merged) d[k.code] = { normalOutput: k.legacyOutput?.normal ?? "", shiftOutput: k.legacyOutput?.shift ?? "", unicodeEquivalent: k.unicodeEquivalent ?? "", status: k.mappingStatus ?? "unverified" };
    setDrafts(d);
  };
  ...
  const doImport = async (file: File) => { const text = await file.text(); await importMappings(JSON.parse(text)); await load(layoutId); };
  if (!unlocked) {
    return (
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5"><Stat label="Total keys" value={summary.totalMappableKeys} /><Stat label="Verified" value={summary.verifiedCount} /><Stat label="Unverified" value={summary.unverifiedCount} /><Stat label="Unsupported" value={summary.unsupportedCount} /><Stat label="Completion" value={`${summary.totalMappableKeys > 0 ? Math.round((summary.verifiedCount / summary.totalMappableKeys) * 100) : 0}%`} /></div>
